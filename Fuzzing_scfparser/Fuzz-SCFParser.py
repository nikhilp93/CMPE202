...
for i in range(N):
        time.sleep(2)
        with open("c:\project\SCFProgram\SCFFile.tlv","wb") as fp:
            for j in range(N):
                data = f.generate() # generate string
                fp.write(data)
                #fp.write(out.stdout.decode('UTF-8'))
                print("(" + str(len(data)) + ")") # print length of string
                print(data) # print string itself

        out=subprocess.run(["SCFParser.exe"],stdout=subprocess.PIPE)
        print(out)
        ...
        if re.search("Buffer Overflow",out.stdout.decode('UTF-8')):
            ...
            target_name="c:\project\SCFProgram\CRASH_INPUTS\SCFFile"+str(st)+".tlv"
